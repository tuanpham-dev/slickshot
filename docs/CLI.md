...
The same binary doubles as a CLI. Non-interactive commands run headless (no window ever opens) and work whether or not the app is already running; `region`/`window`/`open` are interactive — they forward to a running instance, or launch one, and the app stays resident in the tray afterwards.
...
Capture commands (`screen`, `monitor`, `window`, `region`) share output flags:
